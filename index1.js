(function () {
    ...
    const systemKeys = ['Backspace', 'Tab', 'CapsLock', 'ShiftLeft', 'ShiftRight', 'Enter', 'ControlRight', 'AltRight', 'AltLeft', 'ControlLeft', 'MetaLeft', 'Shift', 'Ctrl', 'Alt', 'Win'];
    let lang;
    ...
    const keyboard = document.createElement('div'); // add keyboard div
    ...
    function keyDown(e) {
        ...
        const key = document.querySelectorAll('.' + e.code);
    ...
    function mouseDown({
        ...
        if (parent.tagName === 'DIV') {
            ...
            if (target.children[0].className === 'show') res = target.children[0].innerHTML;
            else res = target.children[1].innerHTML;
        } else {
    ...
    function mouseUp() {
        ...
        if (isShift === 1) {
            if (isShiftKeyboard === 1) {
                document.querySelector('.ShiftLeft').parentElement.classList.add('selected');
            } 
            else if (isShiftKeyboard === 2) document.querySelector('.ShiftRight').parentElement.classList.add('selected');
            else {
                changeCase();
                isShift = 0;
            }
    ...
    function changeCase() {
        const keyS = document.querySelectorAll('.On');
        let activeChild = 0,
            notActiveChild = 0;
